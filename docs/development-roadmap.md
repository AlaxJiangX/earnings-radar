...
> 状态：规划稿。阶段 0–3.4 已完成；阶段 3.2 真实指数 Provider 仍受来源/许可确认门阻塞；阶段 4.1A（EarningsEvent 核心领域模型）、4.1B（EarningsDateChange）、4.1C（EarningsEvent Status Lifecycle）和 4.1D（Candidate Promotion）均已完成，Stage 4.1 财报事件领域基础完成；Stage 4.2A（Earnings Calendar Observation、External Identity 与 Reconciliation Contract Ratification）已完成，ADR-010 已接受；Stage 4.2B（Earnings Calendar Observation & Reconciliation Schema Foundation）已完成；Stage 4.2C（Fixture-First Earnings Calendar Ingestion & Replay）已完成并通过 merge 后复验；Stage 4.2D（Monitoring-Pool Selector & Candidate Foundation）为 IN PROGRESS，4.2D-1 selector/snapshot core 已完成，4.2D-2 candidate/company matching core 已完成、已 merge 并通过 merge 后定向复验；4.2E–4.2F 尚未完成，4.2F live Provider 仍受 license gate 阻塞；SEC Filing（阶段 4.4）、自选股与个人页面（阶段 5）和通知（阶段 6）尚未开始。
>
...
#### 4.2 财报日历 Provider、同步与 Reconciliation（4.2A ✅ COMPLETE；4.2B ✅ COMPLETE；4.2C ✅ COMPLETE；4.2D IN PROGRESS，4.2D-1 COMPLETE；4.2D-2 COMPLETE）
...
4.2D-1 Monitoring-Pool Selector & Snapshot Foundation
4.2D-2 Candidate Creation & Company Matching
...
4.2C ✅ COMPLETE。4.2C-1 已完成：fixture-first parser protocol、synthetic fixture parser 与
parser contract / identity-safety tests 已实现并通过 verification。4.2C-2 已完成：raw-first
ingestion foundation（raw → parse attempt → normalized observation）已实现并通过 verification，
...
4.2C-7 Provider Context Foundation blocker resolution 已完成并通过 targeted verification：
`SyncRun.provider_version` 作为 run-level immutable provenance 持久化；新 earnings-calendar
ingestion 在 raw writes 前强制 provider context，历史 NULL run fail closed 且不具备 replay
eligibility；replay digest 纳入 provider version，caller override 与 normalized inference
均被禁止。4.2C-7 Offline Replay Orchestration 已实现 service entry point、全程 ownership、
deterministic replay observation/parse/normalized persistence、stale resume、count reconciliation、
...
IndexMembership；`monitoring_pool_as_of` 是 `America/New_York` 业务自然日；历史事实采用
Hybrid 模型并持久化 immutable snapshot；retry/replay 复用 persisted pool contract，不重选。
...
4.2D-2 Planning Gate 已完成并接受 ADR-013。规划结论：Candidate 复用现有
`EarningsEvent(identity_status=candidate)`；匹配 canonical unit 为 Company，搜索空间
限定 frozen `MonitoringPoolSnapshot`；matching date 为 `monitoring_pool_as_of`；只允许
...
4.2D-2 ✅ COMPLETE（PR #41，merge `55754b8`，merge 后定向复验通过）。已实现
