...
`indexes.sync_services` 当前只提供离线安全的单快照摄取边界：校验 DataSource/Provider/capability，按幂等键取得 SyncRun 所有权，调用 Provider 后先经 `audit.services` 保存 RawDataRecord/RawDataObservation，再对持久化 bytes 调用注入的来源 parser；解析失败、Provider 失败和空快照都会留下失败运行记录，且不会写 SecurityListing 或 IndexMembership。真实来源确认后仍需在后续 3.2 小阶段补充运行锁、来源专用 Provider/parser、证券核对、差异保护和命令入口。

`companies.services` 是当前 Company 与 SecurityListing 的唯一写入入口。它在 PostgreSQL 事务内规范化 CIK、ticker 与交易所代码，使用数据库唯一/排他约束处理重跑与并发冲突，并在创建时追加 `AuditRecord`、在字段实际变化时通过 `audit.services.record_data_change` 追加 `DataChange`。SecurityListing 的 company、ticker、exchange 与有效期是历史身份，不能原地更新；专用切换用例在同一事务关闭旧 `[start, end)` 区间并创建新的后继 listing。已关闭旧 listing 的重放不能只凭后继存在返回成功：Service 必须只读核对关闭的 DataChange（包括由 audit 公共计算函数重算的 `change_key`）、旧 listing AuditRecord 和后继创建 AuditRecord 的完整身份；任一缺失或不一致均停止并交由人工核查，不伪造或改写审计历史。领域 Service 对 SourceEvidence 只按主键重新加载持久化来源链，并校验证据目标、SyncRun、DataSource 和 RawDataObservation 后才写领域记录或审计。`companies` 不调用 Provider、不创建 SyncRun，也不反向修改 audit 历史；未来同步编排器在原始数据与来源证据已经落库后才调用该领域 Service。
...
### 4.5 财报日历同步与 Reconciliation 契约（4.2A contract ratified；4.2B schema foundation 已实现；4.2C-1 至 4.2C-6 已实现；4.2C-7 Provider Context Foundation 与 offline replay orchestration implementation 已实现）
...
parser；4.2C-2 已实现 raw-first ingestion foundation（raw → parse attempt → normalized
observation）；4.2C-3 已实现 pagination orchestration 与 logical-window completion；
4.2C-4 已实现 canonical earnings-calendar SyncRun scope、scheduled idempotency identity 与
safe/idempotent scheduled SyncRun start。4.2C-5 已实现 `(source, job_type)` PostgreSQL
advisory lock 运行所有权、stale RUNNING 恢复、failed / partial 新运行重试与原 pool hash 核对；
retry 从窗口起点重新抓取，不等于 offline replay。4.2C-6 已接受 ADR-011，并实现受约束
replay run mode / source lineage、contract version、完整 input digest、独立 replay count、
pool contract validation、replay-compatible stale recovery 和 append-only parse/normalized
revision 复用；Option A 只核对 source run 持久化 pool contract，不调用 4.2D selector。
4.2C-7 Provider Context Foundation 已补充 run-level `provider_version` provenance、legacy
NULL replay 拒绝，并把 provider context 纳入 replay digest。以下列表是 ratified contract；
其中完整 offline replay orchestration、company matching、
candidate creation、reconciliation policy、monitoring pool selector 与 live Provider 仍未实现
（余下 4.2C /
4.2D-4.2F）：
...
完整决策见 `docs/decisions/ADR-010-earnings-calendar-observation-and-reconciliation.md` 与
