...
### 4.5 财报日历同步与 Reconciliation 契约（4.2A contract ratified；4.2B schema foundation 已实现；4.2C-1 至 4.2C-6 已实现；offline replay orchestration 尚未实现）
...
revision 复用；Option A 只核对 source run 持久化 pool contract，不调用 4.2D selector。以下
列表是 ratified contract；其中完整 offline replay orchestration、company matching、
