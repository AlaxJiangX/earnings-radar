...
### 4.5 财报日历同步与 Reconciliation 契约（4.2A contract ratified；4.2B schema foundation 已实现；4.2C-1 parser protocol 已实现；4.2C-2 raw-first ingestion foundation 已实现；4.2C+ workflow 未完成）
...
observation）。以下列表是 ratified contract；其中 pagination、scope / scheduled
idempotency、logical window completion、replay、company matching、candidate creation、
reconciliation policy、monitoring pool selector 与 live Provider 仍未实现（4.2C-3+ /
4.2D-4.2F）：
