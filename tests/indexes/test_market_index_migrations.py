...
def _run_forward() -> None:
    call_command("migrate", "indexes", "0001_initial_market_index", "--noinput", stdout=StringIO())
    call_command("migrate", "indexes", "0002_seed_builtin_indexes", "--noinput", stdout=StringIO())
...
class TestSeedMigration:
    ...
    def test_seed_idempotent_rerun(self, db: object) -> None:
        del db
    ...
    def test_seed_preserves_modified_is_enabled(self, db: object) -> None:
        del db
    ...
    def test_seed_accepts_consistent_data(self, db: object) -> None:
        del db
        # Should not raise — all data matches seed expectations
        _call_seed_forward()
